...
export default {
  ...
  methods: {
    async fetchData() {
      ...
      const queryParams = `von=${encodeURIComponent(this.datetimeStart)}&bis=${encodeURIComponent(this.datetimeEnd)}&type=altitude`;
...
this.chartData.datasets[0].data = data.messwerte.map(entry => entry.altitude);

   
        const durchschnitt = data.durchschnitte[0];
        this.durchschnittWert = durchschnitt.altitude_durchschnitt;
        this.minimumWert = durchschnitt.altitude_minimum;
        this.maximumWert = durchschnitt.altitude_maximum;
...
.spinner {
  ...
  border-top: 4px solid rgba(255, 206, 86, 1);
  border-radius: 50%;
  width: 30px;
  height: 30px;
