...
export default {
  ...
  methods: {
    async fetchData() {
      ...
      const queryParams = `von=${encodeURIComponent(this.datetimeStart)}&bis=${encodeURIComponent(this.datetimeEnd)}&type=trip`;
...
this.chartData.datasets[0].data = data.messwerte.map(entry => entry.trip);

        const durchschnitt = data.durchschnitte[0];
        this.durchschnittWert = durchschnitt.trip_durchschnitt;
        this.minimumWert = durchschnitt.trip_minimum;
        this.maximumWert = durchschnitt.trip_maximum;
