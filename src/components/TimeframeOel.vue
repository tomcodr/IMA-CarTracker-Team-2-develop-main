...
export default {
  ...
  methods: {
    async fetchData() {
      ...
      const queryParams = `von=${encodeURIComponent(this.datetimeStart)}&bis=${encodeURIComponent(this.datetimeEnd)}&type=oil_temperature`;
...
this.chartData.datasets[0].data = data.messwerte.map(entry => entry.oil_temperature);

        
        const durchschnitt = data.durchschnitte[0];
        this.durchschnittWert = durchschnitt.oil_temperature_durchschnitt;
        this.minimumWert = durchschnitt.oil_temperature_minimum;
        this.maximumWert = durchschnitt.oil_temperature_maximum;
