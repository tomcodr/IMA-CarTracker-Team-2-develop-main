...
export default {
  ...
  methods: {
    async fetchData() {
      ...
      const queryParams = `von=${encodeURIComponent(this.datetimeStart)}&bis=${encodeURIComponent(this.datetimeEnd)}&type=water_temperature`;
...
this.chartData.datasets[0].data = data.messwerte.map(entry => entry.water_temperature);

        const durchschnitt = data.durchschnitte[0];
        this.durchschnittWert = durchschnitt.water_temperature_durchschnitt;
        this.minimumWert = durchschnitt.water_temperature_minimum;
        this.maximumWert = durchschnitt.water_temperature_maximum;
